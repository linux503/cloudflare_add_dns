...
// header 共用
$header = array(
    "X-Auth-Email:xxxx@gmail.com", // cloudflare账号  
    "X-Auth-Key:xxxxxxxxxxxxxxxxxxx", // cloudflare  key 从 https://dash.cloudflare.com/profile/api-tokens 查看
...
foreach ($domain as $v_domain)
{
    ...
    if ($rs['success'] == false)
    {
        echo '添加失败，错误原因：' . $rs['errors'][0]['message'] . "<br />";
        continue;
    }
    else
    {
        echo '添加域名成功' . "<br />";
        echo '域名id：'     . $rs['result']['id'] . "<br />";
        echo '域名：'       . $rs['result']['name'] . "<br />";
        echo '域名状态：'   . $rs['result']['status'] . "<br />";
        echo '开始添加解析' . "<br />";
        $zoneid = $rs['result']['id'];
    }
    ...
    foreach ($record as $v_record)
    {
        ...
        if ($rs['success'] == false)
        {
            echo '记录添加失败，错误原因：' . $rs['errors'][0]['message'] . "<br />";
        }
        else
        {
            echo '记录添加成功' . $record_detail."<br />";
        }
    }
